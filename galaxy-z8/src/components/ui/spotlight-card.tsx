...
const GlowCard: React.FC<GlowCardProps> = ({
  ...
  const getInlineStyles = (): React.CSSProperties & Record<string, string | number> => {
    const baseStyles: React.CSSProperties & Record<string, string | number> = {
      ...
      '--radius': '14',
      '--border': '2',
      '--backdrop': 'hsl(0 0% 98% / 0.9)',
      '--backup-border': 'rgba(0,0,0,0.08)',
      '--size': '250',
      '--outer': '1',
  ...
  return (
    <>
      ...
      <div
        ...
        className={`${getSizeClasses()} rounded-2xl relative flex flex-col gap-3 overflow-hidden transition-transform duration-300 ease-out hover:-translate-y-1.5 ${className}`}
      >
